...
def rodrigues_rot(normal, vector, theta):
    ...
    return vector*np.cos(theta) + np.cross(normal, vector)*np.sin(theta) + (normal.dot(vector))*normal*(1-np.cos(theta))
...
def trotter_suzuki(t, dt, spin, J):
    ...
    #calculate energy
    engy=np.sum([spin[2*i,:].dot(spin[2*i+1,:]) for i in range(0, int(N/2))])
...
def gs_coeff(v1,v2):
    ...
    return v2.dot(v1)/np.linalg.norm(v1)**2
    # return v2@v1
...
class heis(object):
    ...
    def get_engy0(self):
        ...
        engy0=np.sum([spin[2*i,:].dot(spin[2*i+1,:]) for i in range(0, int(self.N/2))])
...
def calc_lyaps_save(ham,tmax, dt, eps):
    ...
    np.save(savename, np.array([lyap_time, lyap_coeffs]))
